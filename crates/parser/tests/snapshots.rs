...
fn core_program_snapshot() {
    let text = include_str!("programs/core.vlt");
    expect_file!["programs/core.ast"].assert_eq(&parse_dump("core.vlt", text));
}
...
fn syntax_errors_snapshot() {
    ...
    expect_file!["programs/errors.diag"].assert_eq(&parse_dump("errors.vlt", text));
}
...
fn asi_restricted_productions() {
    ...
    let dump = parse_dump("asi.vlt", "function f() {\n    return\n    1 + 2\n}\n");
    ...
    let dump = parse_dump("asi2.vlt", "a\n++b\n");
    assert!(
...
fn nested_type_close_splits_shift_tokens() {
    let dump = parse_dump("v.vlt", "var m:Vector.<Vector.<Vector.<uint>>>;");
    assert!(
...
fn nullable_type_suffix() {
    let dump = parse_dump("n.vlt", "var s:String? = null;");
